    ...
    insert(value) {
        ...
        while (current) {
            ...
            if (value < current.value) {
                ...
                if (current.left === null || current.left.dummy) { // Avoid inserting into dummy nodes
                    current.left = newNode;
                    return this; // Node inserted, break out of the loop
                }
                // Move left
                current = current.left;
            } else {
                // If right child is null, insert here
                if (current.right === null || current.right.dummy) { // Avoid inserting into dummy nodes
...
const BSTVisualization = () => {
    ...
    const resetTree = () => {
        ...
        newBst.addDummyNodes(newBst.root); // Add dummy nodes for structure
    ...
    }, [highlightedPath, bst]); // Add `bst` as a dependency so that it re-renders when the tree is reset
    ...
    // Depth-First Search (DFS) with node highlighting
    const dfs = async (current, value, delay = 1000, path = []) => {
        if (!current || current.data.dummy) return false; // Skip dummy nodes
        
        path.push(current); // Track path for visualization
        ...
        await highlightNode(current, 'checked', delay); // Mark node as checked (red)
        ...
        path.pop(); // Backtrack if value is not found
    ...
    const highlightNode = async (node, status, delay) => {
        console.log(`Highlighting node ${node.data.value} as ${status}`);
